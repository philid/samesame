...
function isPresent(path, items){
	var res = [];
	//console.log("is present : "+name)
	items.forEach(function(e){	
	//	console.log("got items : test : " + name + " - object "+JSON.stringify(e));
		if(retrieve(e, path))
			res.push(e);
	});
	return res;
...
function retrieve(object, path)
{
	...
	//console.log("retrieve last path got : "+JSON.stringify(parts))
	if(tmp)
		res = tmp[parts.shift()];
	///console.log("*****retrieve path got : "+res)
	return res;
}
...
exports.operators = {
	...
	"in": filter(function(value, values){
		//console.log("IN OP : query "+value+" in : "+JSON.stringify(values))
		return values.indexOf(value) > -1;
	}),
	out: filter(function(value, values){
		return values.indexOf(retrieve(value)) == -1;
	}),
	contains: filter(function(array, value){
		if(typeof value == "function"){
			...
			return array instanceof Array && array.indexOf(retrieve(value)) > -1;
		}
	}),
...
function query(query, options, target, prefix){
	options = options || {};
		...
		if(value && typeof value === "object"){
			if(value instanceof Array){
				...
				//	console.log("__________________ GOT OPERTAOR : "+value.name + " - have prefix ? "+prefix)
					///console.log("item['foo.bar'] ==> (item && item.foo && item.foo.bar && ...)")
				...
				//		console.log("Query to js : path will be splitted : "+JSON.stringify(path))

						if(typeof path === 'string')
							path = path.split(".");
				//		console.log("Query to js : path have been splitted : "+JSON.stringify(path))

						var item = "item";
						var escaped = [];
						if(prefix && prefix != "")
							path.unshift(prefix);
				//		console.log("Query to js : path is array : "+JSON.stringify(path))
	...
	var evaluationString = "(function(target){return " + queryToJS(query) + ".call(target);})";
